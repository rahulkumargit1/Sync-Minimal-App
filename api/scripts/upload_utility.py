...
def upload_file_to_s3(file_path, bucket_name, object_name):
    """Upload a file to S3 if it doesn't already exist. Returns the public URL or None."""
    try:
...
async def process_folder(folder_path, upload_to_s3=False):
    ...
    if upload_to_s3 and not S3_BUCKET:
        print("ERROR: S3_BUCKET environment variable is not set. Cannot upload to S3.")
        sys.exit(1)
    ...
    if DATABASE_URL:
        try:
            ...
            print("Continuing with S3 uploads ONLY. Media metadata will not be saved to DB.")
    else:
        print("WARNING: DATABASE_URL is not set. Metadata will not be saved.")
    ...
    for filename in os.listdir(folder_path):
        ...
        if upload_to_s3:
            ...
            uploaded_url = upload_file_to_s3(file_path, S3_BUCKET, s3_key)
            if uploaded_url:
                audio_url = uploaded_url
                print(f"S3 URL: {audio_url}")
            else:
                print(f"Failed to upload {filename}, skipping DB insert.")
                continue
        ...
        if conn:
            try:
                await conn.execute(
                    ...
                    metadata["duration"],
                    audio_url,
                    "https://placeholder.com/cover.jpg",
                )
